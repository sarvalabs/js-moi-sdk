...
class LogicId {
    ...
    constructor(value) {
        if (value.length !== 32) {
            js_moi_utils_1.ErrorUtils.throwArgumentError("Invalid logic id length. Expected 32 bytes.", "value", value);
        }
        this.buff = value;
        const error = LogicId.validate(this);
        if (error != null) {
            js_moi_utils_1.ErrorUtils.throwArgumentError(`Invalid logic identifier. ${error.message}`, "value", value);
        }
    }
    toBytes() {
        return this.buff;
    ...
    toIdentifier() {
        return new identifier_1.Identifier(this.toBytes());
    }
    getTag() {
    ...
    static validate(logicId) {
        ...
        if (tag.getKind() !== identifier_kind_1.IdentifierKind.Logic) {
            return new Error("Invalid identifier kind. Expected a participant identifier.");
        }
        if ((logicId[1] & (flags_1.flagMasks.get(tag.getValue()) ?? 0)) !== 0) {
            return new Error("Invalid participant identifier flags.");
        }
        return null;
    }
    static fromHex(value) {
        return new LogicId((0, js_moi_utils_1.hexToBytes)(value));
    }
}
