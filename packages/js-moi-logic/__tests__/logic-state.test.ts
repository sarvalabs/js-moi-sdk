...
describe("Accessing Logic Storage", () => {
    ...
    test("it should return the size of the map", async () => {
        const length = await logic.logicState.get<number>((accessor) => accessor.entity("Balances"));
    ...
    test("it should able access value of map", async () => {
        ...
        const balance = await logic.logicState.get<string>((accessor) => accessor.entity("Balances").property(address));
    ...
    test("it should be able to access primitive in state", async () => {
        const value = await logic.logicState.get<string>((accessor) => accessor.entity("Symbol"));
