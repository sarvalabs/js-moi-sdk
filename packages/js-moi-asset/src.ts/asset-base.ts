...
export abstract class AssetBase extends ElementDescriptor {
  ...
  protected abstract createPayload(
    ixObject: AssetIxObject,
  ): LogicDeployPayload | LogicActionPayload;

  // TODO: Logic Call Result should be handled seperately
  protected abstract processResult(
    response: AssetIxResponse,
    timeout?: number,
  ...
  protected getTxType(
    kind: string,
  ): OpType.ASSET_CREATE | OpType.ASSET_INVOKE | OpType.LOGIC_ENLIST {
    switch (kind) {
      case "deploy":
  ...
  protected async executeRoutine(
    ...
    if (
      this.getTxType(ixObject.routine.kind) !== OpType.ASSET_CREATE &&
      !this.getLogicId()
    ) {
    ...
    const { type, params } = await this.processArguments(
      ixObject,
      method,
      option,
    );
  ...
  protected createIxObject(
    ...
    ixObject.createPayload = (): AssetCreatePayload | AssetActionPayload => {
      return this.createPayload(ixObject) as
        | AssetCreatePayload
        | AssetActionPayload;
    };
