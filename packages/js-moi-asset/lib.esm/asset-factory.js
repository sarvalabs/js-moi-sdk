...
export class AssetFactory {
    static create(signer, symbol, supply, manager, enableEvents, manifest, callsite, ...calldata) {
        ...
        const option = calldata.at(-1) instanceof RoutineOption ? calldata.at(-1) : undefined;
        ...
        if (callsite == null) {
            if (deployRoutines.length > 0) {
                ErrorUtils.throwError("Manifest defines one or more deploy routines - a callsite is required to select one.", ErrorCode.MISSING_ARGUMENT);
            }
            ...
            if (!builder) {
                ErrorUtils.throwError("No matching deploy routine found in manifest for the given callsite.", ErrorCode.INVALID_ARGUMENT);
            }
