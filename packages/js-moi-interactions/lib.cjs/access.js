...
exports.Access = exports.access = void 0;
const js_moi_utils_1 = require("js-moi-utils");
const context_1 = require("./context");
/** Builds CallerConstraint values for Access.caller()/.origin(). */
exports.access = {
    anyCaller() {
        return { kind: js_moi_utils_1.CallerKind.ANY, set: [] };
    },
    callers(...ids) {
        return { kind: js_moi_utils_1.CallerKind.SET, set: ids };
    },
};
/**
...
class Access {
    ...
    storage(resourceId) {
        this._resource = js_moi_utils_1.ResourceType.STORAGE;
    ...
    policy() {
        ...
        return {
            ...
            caller: this._caller ?? exports.access.anyCaller(),
            origin: this._origin ?? exports.access.anyCaller(),
        };
    }
    create() {
        const policy = this.policy();
        return new PendingAccessOp(js_moi_utils_1.OpType.ACCESS_CREATE, this.signer, (target) => ({
    ...
    update() {
        const policy = this.policy();
        return new PendingAccessOp(js_moi_utils_1.OpType.ACCESS_UPDATE, this.signer, (target) => ({
            target_account: target,
            access_policy: policy,
        }));
    }
    ...
    delete() {
        const { resource, resource_id } = this.resource();
        return new PendingAccessOp(js_moi_utils_1.OpType.ACCESS_DELETE, this.signer, (target) => ({
            target_account: target,
            resource,
            resource_id,
        }));
