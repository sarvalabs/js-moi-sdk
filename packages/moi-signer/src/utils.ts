...
export function bip66Encode(rAndS: SigDigest): Uint8Array {
  let r = rAndS._r;
  let s = rAndS._s;

  let lenR = r.length;
  let lenS = s.length;
  if (lenR === 0) throw new Error(ErrRIsZero);
  if (lenS === 0) throw new Error(ErrSIsZero);
  if (lenR > 33) throw new Error(ErrRIsLong);
  if (lenS > 33) throw new Error(ErrSIsLong);
  if (r[0] & 0x80) throw new Error(ErrRIsNegative);
  if (s[0] & 0x80) throw new Error(ErrSIsNegative);
  if (lenR > 1 && (r[0] === 0x00) && !(r[1] & 0x80)) throw new Error(ErrRIsExcessPad);
  if (lenS > 1 && (s[0] === 0x00) && !(s[1] & 0x80)) throw new Error(ErrSIsExcessPad);
...
export function bip66Decode(buffer: Uint8Array): SigDigest {
  ...
  let lenR = buffer[3];
  if (lenR === 0) throw new Error(ErrRIsZero);
  if (5 + lenR >= buffer.length) throw new Error(ErrSIsLong);
  if (buffer[4 + lenR] !== 0x02) throw new Error(ErrDERExpected2);

  let lenS = buffer[5 + lenR];
  if (lenS === 0) throw new Error(ErrSIsZero);
  if (6 + lenR + lenS !== buffer.length) throw new Error(ErrSIsLong);

  if (buffer[4] & 0x80) throw new Error(ErrRIsNegative);
  if (lenR > 1 && (buffer[4] === 0x00) && !(buffer[5] & 0x80)) throw new Error(ErrRIsExcessPad);

  if (buffer[lenR + 6] & 0x80) throw new Error(ErrSIsNegative);
  if (lenS > 1 && (buffer[lenR + 6] === 0x00) && !(buffer[lenR + 7] & 0x80)) throw new Error(ErrSIsExcessPad);
...
export function JoinSignature(digest: SigDigest): Uint8Array {
