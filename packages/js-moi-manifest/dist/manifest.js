...
class ManifestCoder {
      ...
      manifest.elements.forEach((value) => {
        ...
        switch (value.kind) {
          ...
          case "state":
            ...
            break;
          default:
            js_moi_utils_1.ErrorUtils.throwError(
              `Unsupported kind: ${value.kind}`,
              js_moi_utils_1.ErrorCode.UNSUPPORTED_OPERATION
            );
        }
  ...
  parseCalldata(schema, arg, updateType = true) {
    ...
    const parseStruct = (schema, arg, updateType) => {
      ...
      const doc = (0, js_polo_1.documentEncode)(
        arg,
        reconstructSchema((0, js_moi_utils_1.deepCopy)(schema))
      );
      if (updateType) {
    ...
    switch (schema.kind) {
      ...
      case "map":
        if (
          parsableKinds.includes(schema.fields.keys.kind) ||
  ...
  static decodeException(error) {
    if (error && error !== "0x") {
      ...
      const exception = depolorizer.depolorize(
        schema_1.Schema.PISA_EXCEPTION_SCHEMA
      );
      return exception;
    }
    return null;
