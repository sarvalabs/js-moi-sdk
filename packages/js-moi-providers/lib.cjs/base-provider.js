...
const resourceTypeToLabel = (resourceType) => js_moi_utils_1.ResourceType[resourceType].toLowerCase();
;
/**
...
class BaseProvider extends abstract_provider_1.AbstractProvider {
    ...
    async getSubAccountCount(id, options) {
        try {
            ...
            return (0, js_moi_utils_1.hexToBN)(count);
        }
        catch (error) {
            throw error;
        }
    ...
    *
    ...
    async getLogsFilter(filter) {
        ...
        if (!(0, js_moi_utils_1.isValidAddress)(id)) {
            js_moi_utils_1.ErrorUtils.throwArgumentError("Invalid identifier provided", "identifier", id);
        }
    ...
    async getAccessPolicies(id, resourceType, options) {
        try {
            const params = {
                id,
                resource_type: resourceTypeToLabel(resourceType),
                resource_id: js_moi_constants_1.ZERO_ADDRESS,
                options: options ? options : defaultOptions,
            };
    ...
    async getLogs(logFilter) {
        ...
        if (!(0, js_moi_utils_1.isValidAddress)(id)) {
            js_moi_utils_1.ErrorUtils.throwArgumentError("Invalid identifier provided", "id", id);
        }
        if (!Array.isArray(topics)) {
            js_moi_utils_1.ErrorUtils.throwArgumentError("Topics should be an array", "topics", topics);
        }
