...
export const validateKeyAdd = (key: KeyAddPayload, index: number) => {
  if (typeof key.public_key !== "string" || key.public_key.length === 0) {
    throw new Error(
      `keys[${index}]: public key must be a non-empty hex string`,
    );
  }
...
export const validateParticipantCreate = (
...
export const validateAccountInherit = (payload: AccountInheritPayload) => {
  ...
  if (
    typeof payload.target_account !== "string" ||
  ...
  if (
    typeof payload.sub_account_index !== "number" ||
...
export const validateStorageDeposit = (payload: StoragePayload) => {
  ...
  if (
    typeof payload.deposit_for !== "string" ||
  ...
  if (payload.amount < MIN_STORAGE_DEPOSIT_AMOUNT) {
    throw new Error(
      `amount must be at least ${MIN_STORAGE_DEPOSIT_AMOUNT} (the blockchain's ANUPerByte * StorageMultiplier floor)`,
    );
  }
};
...
export const validateStorageWithdraw = (payload: StoragePayload) => {
  ...
  if (
    payload.bytes_to_release !== undefined &&
    (typeof payload.bytes_to_release !== "number" ||
      payload.bytes_to_release < 0)
  ) {
    throw new Error(
      "bytes_to_release must be a non-negative number if provided (0 or omitted releases everything available)",
    );
  }
};

export const validateCallerConstraint = (
  ...
  if (
    constraint.kind !== CallerKind.ANY &&
  ...
  if (
    ...
    throw new Error(
      `${label}.set must be a non-empty array when kind is CallerKind.SET`,
    );
  }
};
...
export const validateAccessPolicy = (policy: AccessPolicy) => {
  ...
  if (
    typeof policy.resource_id !== "string" ||
...
export const validateLogicPayload = (
  payload: LogicDeployPayload | LogicActionPayload,
) => {
...
export const validateAssetCreate = (payload: AssetCreatePayload) => {
  ...
  if (payload.dimension !== undefined) {
    if (
      typeof payload.dimension !== "number" ||
      payload.dimension < 0 ||
      payload.dimension > 1
    ) {
  ...
  if (
    (typeof payload.max_supply !== "number" &&
  ...
  if (payload.static_metadata) {
    if (
      typeof payload.static_metadata !== "object" ||
    ...
    for (const [k, v] of Object.entries(payload.static_metadata)) {
      if (typeof v !== "string" || v.length === 0) {
        throw new Error(
          `static metadata['${k}'] must be a non-empty hex string`,
        );
      }
    }
  }
  ...
  if (payload.dynamic_metadata) {
    if (
      typeof payload.dynamic_metadata !== "object" ||
    ...
    for (const [k, v] of Object.entries(payload.dynamic_metadata)) {
      if (typeof v !== "string" || v.length === 0) {
        throw new Error(
          `dynamic metadata['${k}'] must be a non-empty hex string`,
        );
      }
    }
  }
...
function processAccountConfigure(payload: AccountConfigurePayload) {
  return polorize(
    { ...payload, add: mapPublicKeys(payload.add) },
    accountConfigureSchema,
  );
}
...
export const processInteractionObject = (
  ix: InteractionObject,
): InteractionObject => {
  return {
    ...ix,
 ...
 */
export const toRawInteractionObject = (
  ix: InteractionObject,
): RawInteractionObject => {
  ...
  return {
    ...
    participants: ix.participants?.map((participant) =>
      toRawParticipant(participant),
    ),
    ix_operations: ix.ix_operations?.map((operation) =>
      toRawOperation(operation),
    ),
