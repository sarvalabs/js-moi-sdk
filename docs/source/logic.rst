...
The logic package in js-moi-sdk is a powerful component that simplifies the 
...
With the logic package, the logic manifests can be easily deployed using logic 
factory. This includes the ability to encode and deploy logics with builder 
arguments.

Once deployed, the package handles routine calls, interaction sending, and 
state queries, abstracting away complexities like parameter encoding, fuel and 
interaction signing.

Integration with the wallet and signer packages ensures secure interaction 
...
The logic package is designed to be simple and intuitive. The following example
